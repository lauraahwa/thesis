...
function cap_reserve_margin!(EP::Model, inputs::Dict, setup::Dict)
    ...
    if haskey(inputs, "dfCapRes_slack")
        ...
        add_simila,r_to_expression!(EP[:eCapResMarBalance], vCapResSlack)
    ...
    if get(setup, "DCCleanCase", 0) > 0
        dc_res = get(setup, "DCCleanCapResZone", 8)   # CapRes_8 = DC Island
        clean_share = setup["DCCleanShare"]
        capresfactor = inputs["DERATING_FACTOR"]
        names = inputs["RESOURCE_NAMES"]

        DC = [y for y in eachindex(names) if capresfactor[y, dc_res] != 0]

        if !isempty(DC)
            DC_VRE = intersect(DC, inputs["VRE"])
            DC_STOR = intersect(DC, inputs["STOR_ALL"])
            DC_THERM = intersect(DC, inputs["THERM_ALL"])

            is_nuclear(y) = occursin("nuclear", lowercase(String(names[y])))
            ...
            for t in 1:T
                for y in DC_THERM_CLEAN
                    add_to_expression!(EP[:eDCCleanCapRes][t],
                        capresfactor[y, dc_res], EP[:eTotalCap][y])
                end
                for y in DC_VRE
                    add_to_expression!(EP[:eDCCleanCapRes][t],
                        capresfactor[y, dc_res] * inputs["pP_Max"][y, t], EP[:eTotalCap][y])
                end
            end

            @expression(EP, eDCOnsiteCapRes[t = 1:T], AffExpr(0.0))
            for t in 1:T
                for y in DC_GAS
                    add_to_expression!(EP[:eDCOnsiteCapRes][t],
                        capresfactor[y, dc_res], EP[:eTotalCap][y])
                end
                for y in DC_STOR
                    add_to_expression!(EP[:eDCOnsiteCapRes][t],
                        capresfactor[y, dc_res], EP[:vP][y, t])
                    add_to_expression!(EP[:eDCOnsiteCapRes][t],
                        -capresfactor[y, dc_res], EP[:vCHARGE][y, t])
                    if setup["StorageVirtualDischarge"] > 0
                        add_to_expression!(EP[:eDCOnsiteCapRes][t],
                            capresfactor[y, dc_res], EP[:vCAPRES_discharge][y, t])
                        add_to_expression!(EP[:eDCOnsiteCapRes][t],
                            -capresfactor[y, dc_res], EP[:vCAPRES_charge][y, t])
                    end
                end
            end

            @expression(EP, eDCCapResReq[t = 1:T],
                sum(inputs["pD"][t, z] * (1 + inputs["dfCapRes"][z, dc_res])
                    for z in zones_dc))
            # clean share would be set to 0.9 for 90% clean capacity requirement
            @constraint(EP, cDCCleanFloor[t = 1:T],
                EP[:eDCCleanCapRes][t] >= clean_share * EP[:eDCCapResReq][t])

            @constraint(EP, cDCOnsiteCap[t = 1:T],
                EP[:eDCOnsiteCapRes][t] <= (1 - clean_share) * EP[:eDCCapResReq][t])
        end
    end
